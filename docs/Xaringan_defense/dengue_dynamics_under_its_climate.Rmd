...
Dengue has the following characteristic:
  ...
  - is a vector borne disease, human-mosquito-human cycle

  - has an incubation period of four- to six-day and persists until around the time fever abates, 
which is typically three to seven days, potential of cryptic transmissions

  - is a hyperendemic disease in Brazil, with anual cycles of outbreaks
...
**How temperatures & total preciptation affects the counts of cases due to dengue in Rio de Janeiro, during 2010-2019?**
...
To employ the Takens theorem in practice, we seek for points in one __Shadow-Manifold__, say $\mathbf{M}_X$ with the same temporal indexes, into the other, say $\mathbf{M}_Y$, and we follow ahead those points $n$ steps, to give a prediction to the future of points into the other manifold, in a equation:

$$
\begin{equation}
\hat{Y}(t+n)|\mathbf{M_X} = \sum_{i=1}^{E+1}w_iY_i(t+n)
\end{equation}
$$

Repeating this mapping for the complete set of points of each time series, we can have a prediction of $Y$ given the points of $X$, and vice-versa

---

## Causality criteria

With the last equation, we can give a prediction for $Y$ given points in $\mathbf{M}_X$, the comparison between the observed time series of $Y(t)$ and the predicted time series $\hat{Y}(t)|\mathbf{M}_X$, is the measure of how strong linked each __Shadow-Manifold__ is to the other:

$$
\begin{equation}
\rho = \rho_{Y,\hat{Y}|\mathbf{M_X}} = \rho_{CCM}(L) = \langle \frac{Cov[Y,\hat{Y}|\mathbf{M_X}]}{\sigma_Y\sigma_{\hat{Y}|\mathbf{M_X}}} \rangle(L)
\end{equation}
$$

If this correlation converges to a value, given the totality of points, $L$, this is the criteria for the causality and linking between the both __Shadow-Manifolds__.

---

## Time for prediction

Another part of the EDM framework is to know how much lagged a climate driver that forces into the cases, to do so we test different time for prediction, $tp$, which are different horizons for the mapping between the __Shadow-manifold__, modifying the CCM equation we have:

$$
\begin{equation}
\hat{Y}(t+n+tp)|\mathbf{M_X} = \sum_{i=1}^{E+1}w_iY_i(t+n)
\end{equation}
$$

Where $tp$ is how much ahead of the contemporaneous time we are trying to predict, or how much the relation is lagged from the contemporaneous time, after testing different numbers of $tp$, we end with a distribution of $\rho_{CCM}$ for different lags

---

## Dengue in Rio de Janeiro city

The typical procedure of *EDM*
  ...
  4) Where in the $tp$ the original is above the 0.95th percentile of the $\rho_{CCM}$ distribution for the 500 surrogates time-series, it is a significant causal relation or __driver__
...
To create each surrogate time series, we pick the seasonality pattern and add a sample of residuals randomly shuffled:
  ...
  - When putting this on the CCM function, we ask if the seasonal pattern is driving the dynamics 
...
## Conclusion & Interpretation
  ...
  * __Minimum Precipation__ has a lagged effect on cases of 7 week, in the same shape as Max. 
    * Preciptation, after drought any amount of precipitation helps the mosquito cycle. 
  
  * __Temperatures__ are climate drivers of cases at Rio de Janeiro city, but drives cases in the city by its seasonality and not by its variability, due to not being found as drivers by CCM. 
    * So they are a conditions that is matched in a season on year
...
On this thesis, we investigated the relation of two climate variables with dengue disease, over Brazil during the a 10 years period, from 2010 to 2019
  ...
  - This thesis hinders the establishment of a more mechanistic interpretation, as no data about mosquito abundance is used on this thesis
