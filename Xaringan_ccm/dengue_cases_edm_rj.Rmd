...
If $\theta$ is close to $0$ the model understand the influence of each driver as almost linear, as $\theta$ grows the model gives more non-linear relations to the coefficients and the drivers.

---

```{r theta, echo=FALSE, message=FALSE, warning=FALSE, out.width=750, fig.align='center'}
knitr::include_graphics('img/theta_min_mae_rmse.png')
```
...
$$
\begin{equation}
Cases(t) = C_0(t) + \frac{\partial T_{min}(t - 2)}{\partial Cases(t)}T_{min}(t - 2) + \frac{\partial T_{mean}(t - 2)}{\partial Cases(t)}T_{mean}(t - 2) + \\
\frac{\partial Precip_{min}(t - 8 )}{\partial Cases(t)}Precip_{min}(t - 8)
\end{equation}
$$

The form to each of the forces of interactions $C_i(t)$ are the solution of SVD problem and are given by the choice of theta, which determine how much it nearby state (point) weights to the prediction, here the series of Cases.

---

```{r forces, echo=FALSE, message=FALSE, warning=FALSE, out.width=750, fig.align='center'}
knitr::include_graphics('img/effects_all_drivers.png')
```

---

```{r forces-ts, echo=FALSE, message=FALSE, warning=FALSE, out.width=750, fig.align='center'}
knitr::include_graphics('img/ts_colors_week.png')
```

---

```{r forces-ts-zoom, echo=FALSE, message=FALSE, warning=FALSE, out.width=750, fig.align='center'}
knitr::include_graphics('img/ts_colors_week_zoom.png')
```
